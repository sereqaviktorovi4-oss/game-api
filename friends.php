<?php
// 1. Подключаем db.php (он сам запускает сессию и коннектится к БД)
include 'db.php';
...
// 3. Логика удаления друга
if (isset($_GET['delete'])) {
    $f_id = (int)$_GET['delete'];
    $del_query = "DELETE FROM friends WHERE (user_id = $my_id AND friend_id = $f_id) OR (user_id = $f_id AND friend_id = $my_id)";
    pg_query($db, $del_query);
    pg_close($db);
    header("Location: friends.php");
    exit;
}

// 4. Подключаем шапку сайта ПОСЛЕ редиректов
include 'header.php';

// 5. Запрос для PostgreSQL
$friends_query_str = "
...
$friends_res = pg_query($db, $friends_query_str);
?>

<h2 style="color: var(--accent); margin-bottom: 20px;"><i class="fa-solid fa-user-group"></i> Мои друзья</h2>

<div style="display: grid; grid-template-columns: 1fr; gap: 15px;">
    <?php if ($friends_res && pg_num_rows($friends_res) > 0): ?>
        <?php while($f = pg_fetch_assoc($friends_res)): ?>
            <?php 
                $is_online = false;
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; background: #252530; padding: 15px; border-radius: 15px; border: 1px solid <?php echo $is_online ? 'var(--accent)' : '#333'; ?>;">
                ...
                <div style="display: flex; align-items: center; gap: 15px;">
                    <div style="position: relative; width: 50px; height: 50px;">
                        <?php if(!empty($f['avatar_path'])): ?>
                            <img src="<?php echo htmlspecialchars($f['avatar_path']); ?>" style="width: 100%; height: 100%; border-radius: 50%; object-fit: cover; border: 2px solid <?php echo $is_online ? '#00ff00' : 'var(--primary)'; ?>;">
                        <?php else: ?>
                        ...
                        <?php if($f['unread_msgs'] > 0): ?>
                            <div style="position: absolute; top: -5px; right: -5px; background: #ff4444; color: white; font-size: 0.7rem; padding: 2px 7px; border-radius: 10px; border: 2px solid #252530; animation: pulse 1.5s infinite;">
                                +<?php echo (int)$f['unread_msgs']; ?>
                            </div>
                        <?php endif; ?>
                    </div>
                    
                    <div>
                        <a href="profile.php?id=<?php echo (int)$f['id']; ?>" style="text-decoration: none; color: #fff; font-weight: bold;"><?php echo htmlspecialchars($f['username']); ?></a>
                        <div style="font-size: 0.75rem; color: <?php echo $is_online ? 'var(--accent)' : '#777'; ?>;">
                            <?php 
                                if ($is_online) {
                                    echo (isset($f['platform']) && $f['platform'] == 'game') ? '🎮 В игре Love City' : '🌐 На сайте';
                                } else {
                                    echo 'Офлайн';
                                }
                            ?>
                ...
                <div style="display: flex; gap: 10px; align-items: center;">
                    <?php if ($is_online && isset($f['platform']) && $f['platform'] == 'game'): ?>
                        <a href="lovecity://visit=<?php echo (int)$f['id']; ?>" class="btn-action" style="padding: 8px 15px; font-size: 0.75rem; text-decoration: none; background: #ff00ff; color: white; border: none;" title="Телепортироваться к другу в игре">
                            <i class="fa-solid fa-plane"></i> В гости
                        </a>
                    <?php endif; ?>

                    <a href="messages.php?to=<?php echo (int)$f['id']; ?>" class="btn-action" style="padding: 8px 15px; font-size: 0.75rem; text-decoration: none; background: <?php echo ($f['unread_msgs'] > 0) ? 'var(--accent)' : 'var(--primary)'; ?>; <?php echo ($f['unread_msgs'] > 0) ? 'color: #000;' : ''; ?>">
                        <i class="fa-solid fa-envelope"></i> Читать
                    </a>
                    <a href="friends.php?delete=<?php echo (int)$f['id']; ?>" style="color: #555; font-size: 1.1rem; padding: 5px;" onclick="return confirm('Удалить из друзей?')" title="Удалить">
