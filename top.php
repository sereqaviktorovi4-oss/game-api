...
if (isset($_GET['add_friend']) && $my_id) {
    ...
    $check = $db->query("SELECT id FROM friends WHERE (user_id=$my_id AND friend_id=$friend_id) OR (user_id=$friend_id AND friend_id=$my_id)");
    
    if ($check->num_rows == 0 && $my_id != $friend_id) {
        // Добавляем запись (для простоты сразу подтвержденную)
        $db->query("INSERT INTO friends (user_id, friend_id, status) VALUES ($my_id, $friend_id, 'accepted')");
...
$users = $db->query("SELECT id, username, gender, birth_year, plot_coords FROM users $search_query ORDER BY id DESC LIMIT 50");
?>
        ...
        <tbody>
            <?php if ($users->num_rows > 0): ?>
                ...
                <tr style="border-bottom: 1px solid #333; transition: 0.3s;" onmouseover="this.style.background='#252530'" onmouseout="this.style.background='none'">
                    ...
                    <td style="padding: 12px;"><?php echo $row['birth_year']; ?></td>
                    <td style="padding: 12px; color: #aaa;">
                        <i class="fa-solid fa-location-dot"></i> 
                        <?php echo $row['plot_coords'] ? $row['plot_coords'] : 'Нет дома'; ?>
                    </td>
                    <td style="padding: 12px; text-align: center;">
                        <?php if ($my_id && $my_id != $row['id']): ?>
                            <a href="top.php?add_friend=<?php echo $row['id']; ?>" class="btn-action" style="padding: 5px 10px; text-decoration: none; font-size: 0.8rem;" title="Добавить в друзья">
